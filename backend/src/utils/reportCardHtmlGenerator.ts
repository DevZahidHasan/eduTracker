...
export const generateReportCardHtml = (data: any, schoolProfile: any) => {
  ...
  const obtainedMarks = marks.reduce((acc: number, m: any) => acc + m.score, 0);
  const totalMax = marks.reduce((acc: number, m: any) => acc + m.maxScore, 0);
